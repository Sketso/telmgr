...
if command -v telmgr &>/dev/null; then
    ...
    if [[ "$DO_UPGRADE" =~ ^[Yy]$ ]]; then
        ...
        curl -Ls https://raw.githubusercontent.com/Sketso/telmgr/feat/mtproto-zig-engine/telmgr -o /usr/local/bin/telmgr
        ...
        if [ -f "$BOT_PY" ]; then
            ...
            curl -Ls https://raw.githubusercontent.com/Sketso/telmgr/feat/mtproto-zig-engine/bot/bot.py -o "$BOT_PY"
            ok "bot.py updated"
...
if $INSTALL_BOT_ENABLED && ! $BOT_SLAVE; then
    ...
    curl -Ls https://raw.githubusercontent.com/Sketso/telmgr/feat/mtproto-zig-engine/bot/bot.py -o "$TELEMT_DIR/bot.py"
